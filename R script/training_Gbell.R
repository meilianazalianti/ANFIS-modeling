...
data <- readxl::read_excel("C:/Users/A c e r/OneDrive/Documents/skripsi/energy+efficiency/data_latih90.xlsx")
data
...
train_ANFIS <- function(data,epoch,learning_rate){
  ...
  for (iter in seq_len(epoch))
    rest_list[[iter]] <- {
      ...
      A1 <- gbell(data$x1, as.numeric(std[1, 2]), 1, as.numeric(Mean[1, 2]))
      A2 <- gbell(data$x1, as.numeric(std[2, 2]), 1, as.numeric(Mean[2, 2]))
      B1 <- gbell(data$x2, as.numeric(std[1, 3]), 1, as.numeric(Mean[1, 3]))
      B2 <- gbell(data$x2, as.numeric(std[2, 3]), 1, as.numeric(Mean[2, 3]))
      C1 <- gbell(data$x3, as.numeric(std[1, 4]), 1, as.numeric(Mean[1, 4]))
      C2 <- gbell(data$x3, as.numeric(std[2, 4]), 1, as.numeric(Mean[2, 4]))
      D1 <- gbell(data$x4, as.numeric(std[1, 5]), 1, as.numeric(Mean[1, 5]))
      D2 <- gbell(data$x4, as.numeric(std[2, 5]), 1, as.numeric(Mean[2, 5]))
      E1 <- gbell(data$x5, as.numeric(std[1, 6]), 1, as.numeric(Mean[1, 6]))
      E2 <- gbell(data$x5, as.numeric(std[2, 6]), 1, as.numeric(Mean[2, 6]))
      F1 <- gbell(data$x6, as.numeric(std[1, 7]), 1, as.numeric(Mean[1, 7]))
      F2 <- gbell(data$x6, as.numeric(std[2, 7]), 1, as.numeric(Mean[2, 7]))
