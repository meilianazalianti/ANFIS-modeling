...
#read datasets
df <- readxl::read_excel("C:/Users/A c e r/OneDrive/Documents/skripsi/energy+efficiency/cooling.xlsx")
...
A1 <- sigmoid(data$x1,step,as.numeric(Mean[1, 2]))
A2 <- sigmoid(data$x1,step,as.numeric(Mean[2, 2]))
B1 <- sigmoid(data$x2,step,as.numeric(Mean[1, 3]))
B2 <- sigmoid(data$x2,step,as.numeric(Mean[2, 3]))
C1 <- sigmoid(data$x3,step,as.numeric(Mean[1, 4]))
C2 <- sigmoid(data$x3,step,as.numeric(Mean[2, 4]))
D1 <- sigmoid(data$x4,step,as.numeric(Mean[1, 5]))
D2 <- sigmoid(data$x4,step,as.numeric(Mean[2, 5]))
E1 <- sigmoid(data$x5,step,as.numeric(Mean[1, 6]))
E2 <- sigmoid(data$x5,step,as.numeric(Mean[2, 6]))
F1 <- sigmoid(data$x6,step,as.numeric(Mean[1, 7]))
F2 <- sigmoid(data$x6,step,as.numeric(Mean[2, 7]))
...
# Matriks Desain A
A <- matrix(c(c11, c12, c13, c14, c15, c16, c10, c21, c22, c23, c24, c25,c26, c20), 614, 14)
A
...
library(MASS)
BI <- ginv(B)  # Menghitung pseudo-inverse dari B
...
Tabel
options(max.print = 100000) # atau angka yang lebih besar dari jumlah total baris
print(hasil)

# Scatter plot
plot(hasil$Y, hasil$O_5, 
     xlab = "Nilai Aktual", 
     ylab = "Nilai Prediksi", 
     main = "Perbandingan Aktual vs Prediksi",
     pch = 16, col = "blue")
abline(0, 1, col = "red", lty = 2) # Garis y = x untuk referensi

# Line plot untuk tren
plot(hasil$Y, type = "l", col = "blue", lwd = 2, ylab = "Nilai", xlab = "Indeks",
     main = "Perbandingan Aktual dan Prediksi")
lines(hasil$O_5, col = "red", lwd = 2)
...
##Perbaikan Parameter Premis dengan Model Propagasi error
#Layer 5
...
# Hitung rata-rata variabel input
avrx1 <- mean(data$x1)
avrx2 <- mean(data$x2)
avrx3 <- mean(data$x3)
avrx4 <- mean(data$x4)
...
x1c <- delta_c (0.1,x11, avrx1)
x1c2 <- delta_c (0.1,x12,avrx1)
x2c <- delta_c (0.1,x21, avrx2)
x2c2 <- delta_c (0.1,x22,avrx2)
x3c <- delta_c (0.1,x31, avrx3)
x3c2 <- delta_c (0.1,x32,avrx3)
x4c <- delta_c (0.1,x41, avrx4)
x4c2 <- delta_c (0.1,x42,avrx4)
x5c <- delta_c (0.1,x51, avrx5)
x5c2 <- delta_c (0.1,x52,avrx5)
x6c <- delta_c (0.1,x61,avrx6)
x6c2 <- delta_c (0.1,x62,avrx6)
...
x1a <- delta_a (0.1,x11a)
x1a2 <- delta_a (0.1,x12a)
x2a <- delta_a (0.1,x21a)
x2a2 <- delta_a (0.1,x22a)
x3a <- delta_a (0.1,x31a)
x3a2 <- delta_a (0.1,x32a)
x4a <- delta_a (0.1,x41a)
x4a2 <- delta_a (0.1,x42a)
x5a <- delta_a (0.1,x51a)
x5a2 <- delta_a (0.1,x52a)
x6a <- delta_a (0.1,x61a)
x6a2 <- delta_a (0.1,x62a)
